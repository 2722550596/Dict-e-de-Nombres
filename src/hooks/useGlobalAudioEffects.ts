...
import { playSound, initAudioOnUserInteraction } from '../utils/audioEffects';
...
export const useGlobalAudioEffects = (options: UseGlobalAudioEffectsOptions = {}) => {
  ...
  // 播放交互音效的便捷函数
  const playInteractionSound = useCallback((type: 'click' | 'hover' | 'input' | 'submit' | 'select' | 'tab' | 'navigation' | 'toggle') => {
    playSound(type);
  }, []);
